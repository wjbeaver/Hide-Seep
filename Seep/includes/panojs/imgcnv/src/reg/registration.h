...
// Main driver
int register_image_pair(TDimImage *img, TDimImage *imG, Params *regParams);
...
public:
  ...
  Matrix(const TDimImage *img): cols(0), rows(0), data(0) { fromImage(img); }
...
public:
  ...
  bool      fromImage(const TDimImage *img);
  TDimImage toImage();
...
void copyImageToMatrix(const TDimImage *img, const double &minpv, const double &maxpv, reg::Matrix<Tm> *m) {
  Tm **g = m->data;
...
template <typename T>
bool reg::Matrix<T>::fromImage(const TDimImage *img) {
  this->init( img->height(), img->width() );

  DImageHistogram h(*img);
  double minpv = h[0]->min_value();
  double maxpv = h[0]->max_value(); 

  if (img->depth()==8 && img->pixelType()==D_FMT_UNSIGNED)
    copyImageToMatrix<T, DIM_UINT8>( img, minpv, maxpv, this );
  else
  if (img->depth()==16 && img->pixelType()==D_FMT_UNSIGNED)
    copyImageToMatrix<T, DIM_UINT16>( img, minpv, maxpv, this );
  else
  if (img->depth()==32 && img->pixelType()==D_FMT_UNSIGNED)
    copyImageToMatrix<T, DIM_UINT32>( img, minpv, maxpv, this );
  else
  if (img->depth()==8 && img->pixelType()==D_FMT_SIGNED)
    copyImageToMatrix<T, DIM_INT8>( img, minpv, maxpv, this );
  else
  if (img->depth()==16 && img->pixelType()==D_FMT_SIGNED)
    copyImageToMatrix<T, DIM_INT16>( img, minpv, maxpv, this );
  else
  if (img->depth()==32 && img->pixelType()==D_FMT_SIGNED)
    copyImageToMatrix<T, DIM_INT32>( img, minpv, maxpv, this );
  else
  if (img->depth()==32 && img->pixelType()==D_FMT_FLOAT)
    copyImageToMatrix<T, DIM_FLOAT32>( img, minpv, maxpv, this );
  else
  if (img->depth()==64 && img->pixelType()==D_FMT_FLOAT)
    copyImageToMatrix<T, DIM_FLOAT64>( img, minpv, maxpv, this );

  return 0;
}

template <typename T>
TDimImage reg::Matrix<T>::toImage() {
  TDimImage img(this->cols, this->rows, 8, 1);
  T **g = this->data;
