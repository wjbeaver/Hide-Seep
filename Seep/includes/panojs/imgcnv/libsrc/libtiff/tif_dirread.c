...
static enum TIFFReadDirEntryErr TIFFReadDirEntryPersampleShort(TIFF* tif, TIFFDirEntry* direntry, uint16* value)
{
	...
	uint16* na;
	uint16 nb;
	if (direntry->tdir_count<(uint64)tif->tif_dir.td_samplesperpixel) // dima: changed != to <
		return(TIFFReadDirEntryErrCount);
...
/* END REV 4.0 COMPATIBILITY */
    ...
    // dima: try to recover instead of punting directly
    // dima: gracefully accept as a GRAY or an RGB image in case palette is missing
