...
#ifndef DIM_XPOINTER_H
#define DIM_XPOINTER_H
...
template <typename T> 
class DUniqueReferences {
public:
  typedef void (*DReferencesExhausted)( const T &v );

  DUniqueReferences() { init(); }
  DUniqueReferences(const T &v) { init(v); }
  ~DUniqueReferences() { removeReference(it_current); }
...
public:
  DReferencesExhausted onReferencesExhausted;
