...
std::string DTagMap::readline( const std::string &str, int &pos ) {
  std::string line;
...
void DTagMap::parse_ini( const std::string &ini, const std::string &separator, const std::string &prefix, const std::string &stop_at_dir ) {
  ...
  int pos=0;
  while (pos<ini.size()) {
    std::string line = DTagMap::readline( ini, pos );
...
std::deque<std::string> DTagMap::iniGetBlock( const std::string &ini, const std::string &dir_name ) {
...
//******************************************************************************
std::string DTagMap::join( const std::string &line_sep, const std::string &tag_val_sep  ) const {
  std::string s;
  std::map< std::string, std::string >::const_iterator it = this->begin();
  while (it != this->end()) {
    s += it->first;
...
bool DTagMap::toFile( const std::string &file_name ) const {
  std::ofstream f;
...
bool DTagMap::fromFile( const std::string &file_name ) {
