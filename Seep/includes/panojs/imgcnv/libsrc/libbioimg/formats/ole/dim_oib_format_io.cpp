...
DTagMap parse_lut_stream( ole::Params *par, int sample ) {
...
int oibGetImageInfo( TDimFormatHandle *fmtHndl ) {
  ...
  for (int i=0; i<(int) bim::NumberDisplayChannels; ++i) par->display_lut[i] = -1;
  ...
  for (unsigned int sample=0; sample<info->samples; ++sample) {
    DTagMap lut_info = parse_lut_stream( olePar, sample );
    int r = lut_info.get_value_int( "Red/Contrast", 0 );   // max: 100
    int g = lut_info.get_value_int( "Green/Contrast", 0 ); // max: 100
    int b = lut_info.get_value_int( "Blue/Contrast", 0 );  // max: 100
