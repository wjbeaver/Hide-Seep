...
int zviGetImageInfo( TDimFormatHandle *fmtHndl ) {
  ...
  if (z->pixelFormat()==zvi::PT_FLOAT32 || z->pixelFormat()==zvi::PT_FLOAT64) 
...
void get_z_t( zvi::Directory *zvi_dir, int page, int sample, int &z, int &t ) {
...
static int zvi_read_image(TDimFormatHandle *fmtHndl) {
  if (fmtHndl == NULL) return 1;
  if (fmtHndl->internalParams == NULL) return 1;
  ole::Params *par = (ole::Params *) fmtHndl->internalParams;
  TDimImageInfo *info = &par->i; 
  ...
  zvi::Directory *zvi_dir = &par->zvi_dir;
  ...
  for (unsigned int sample=0; sample<img->i.samples; ++sample) {
    dimProgress( fmtHndl, sample+1, img->i.samples, "Reading ZVI" );
...
DIM_UINT zvi_append_metadata (TDimFormatHandle *fmtHndl, DTagMap *hash ) {
  if (fmtHndl == NULL) return 1;
  if (fmtHndl->internalParams == NULL) return 1;
  if (!hash) return 1;
  ole::Params *par = (ole::Params *) fmtHndl->internalParams;
