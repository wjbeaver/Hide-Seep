...
bool TDimFormatManager::sessionIsReading(const std::string &fileName) const {
  if (!session_active) return false;
  if (fileName.size()>0)
    if (fileName != sessionFileName) return false;
  if (sessionHandle.io_mode == DIM_IO_READ) return true;
  return false;
}

bool TDimFormatManager::sessionIsWriting(const std::string &fileName) const {
  if (!session_active) return false;
  if (fileName.size()>0)
    if (fileName != sessionFileName) return false;
  if (sessionHandle.io_mode == DIM_IO_WRITE) return true;
  return false;
}
