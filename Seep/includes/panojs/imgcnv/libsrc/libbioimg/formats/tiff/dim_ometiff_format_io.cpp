...
DIM_UINT append_metadata_omeTiff (TDimFormatHandle *fmtHndl, DTagMap *hash ) {
  ...
  hash->append_tag( bim::IMAGE_NUM_Z, info->number_z );
  hash->append_tag( bim::IMAGE_NUM_T, info->number_t );
  ...
  for (unsigned int i=0; i<ome->channels; ++i) {
    ...
    if (wavelength.size()>0)
      hash->append_tag( xstring::xprintf(bim::CHANNEL_NAME_TEMPLATE.c_str(), i), medium+" - "+wavelength+"nm" );
    else
      hash->append_tag( xstring::xprintf(bim::CHANNEL_NAME_TEMPLATE.c_str(), i), medium );
  }
  ...
  for (unsigned int i=0; i<ome->channels; ++i) {
    ...
    if (wavelength.size()>0)
      hash->append_tag( xstring::xprintf(bim::CHANNEL_NAME_TEMPLATE.c_str(), chan), medium+" - "+wavelength+"nm" );
    else
      hash->append_tag( xstring::xprintf(bim::CHANNEL_NAME_TEMPLATE.c_str(), chan), medium );
  ...
  while (p != std::string::npos) {
    ...
    if (tag.size()>0) {
      ...
      if (tag.size()>0) {
        ...
        hash->append_tag( xstring::xprintf( bim::STAGE_POSITION_TEMPLATE_X.c_str(), page), sx );
        hash->append_tag( xstring::xprintf( bim::STAGE_POSITION_TEMPLATE_Y.c_str(), page), sy );
        hash->append_tag( xstring::xprintf( bim::STAGE_POSITION_TEMPLATE_Z.c_str(), page), sz );
      }
...
std::string constructOMEXML( TDimFormatHandle *fmtHndl, DTagMap *hash ) {
  ...
  if (hash && hash->size()>0) {
    if ( hash->hasKey(bim::PIXEL_RESOLUTION_X) )
      str += xstring::xprintf(" PhysicalSizeX=\"%s\"", hash->get_value(bim::PIXEL_RESOLUTION_X).c_str() );
    if ( hash->hasKey(bim::PIXEL_RESOLUTION_Y) )
      str += xstring::xprintf(" PhysicalSizeY=\"%s\"", hash->get_value(bim::PIXEL_RESOLUTION_Y).c_str() );
    if ( hash->hasKey(bim::PIXEL_RESOLUTION_Z) )
      str += xstring::xprintf(" PhysicalSizeZ=\"%s\"", hash->get_value(bim::PIXEL_RESOLUTION_Z).c_str() );
    if ( hash->hasKey(bim::PIXEL_RESOLUTION_T) )
      str += xstring::xprintf(" TimeIncrement=\"%s\"", hash->get_value(bim::PIXEL_RESOLUTION_T).c_str() );
  }
  ...
  if (hash && hash->size()>0 && hash->hasKey(bim::CHANNEL_NAME_0)) {
    for (int i=0; i<img->i.samples; ++i) {
      xstring key = xstring::xprintf(bim::CHANNEL_NAME_TEMPLATE.c_str(), i);
      if (hash->hasKey(key)) {
  ...
  if (hash && hash->size()>0) {
    ...
    for(it = hash->begin(); it != hash->end(); ++it) {
      ...
      if (key.startsWith(bim::CUSTOM_TAGS_PREFIX)) {
