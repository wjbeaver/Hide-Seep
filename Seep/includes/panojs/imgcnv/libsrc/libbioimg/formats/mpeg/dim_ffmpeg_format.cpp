...
DIM_INT dimFFMpegValidateFormatProc (DIM_MAGIC_STREAM *magic, DIM_UINT length) {
  ...
  // mpeg2 stream M2T
  if (mag_num[0] == 0x47) return 0; // RAW MPEG2 M2T
...
TDimFormatHandle dimFFMpegAquireFormatProc( void ) {
  ...
  encoder_ids.push_back( CODEC_ID_H264 );  // 13 "MPEG4 H264",
  encoder_ids.push_back( CODEC_ID_NONE );  // 14 "WebM VP8",
  ...
  formats_write.push_back( "h264" );       // 13 "H264",
  formats_write.push_back( "webm" );       // 14 "WebM",
  ...
  formats_fourcc.push_back( "" );     // 13 "H264",
  formats_fourcc.push_back( "" );     // 14 "WebM",
  ...
  formats.insert( std::make_pair( "h264", 13 ) );
  formats.insert( std::make_pair( "webm", 14 ) );
...
#define DIM_FFMPEG_NUM_FORMATS 15
