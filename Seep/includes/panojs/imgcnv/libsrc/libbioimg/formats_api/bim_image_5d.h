...
class Image5D {
  ...
  public:
    ...
    TDimImage   *imageAt( unsigned int t, unsigned int z );
    DImageStack *stackAt( unsigned int t );
    //inline TDimImage *operator[](unsigned int p) const { return imageAt(p); }

    DImageStack *stack(); // stack at the currently selected position
    const DImageStack *current() const; // difference with stack() is that current() is not going to load the image
    TDimImage *image(); // image at the currently selected position
    void resetPosition();
    bool nextTimePoint();
    bool prevTimePoint();
    ...
    const DTagMap* getMetadata() const { return &metadata; }
  ...
  protected:
    ...
    std::vector<DImageStack> stacks;
    std::list<DImageStack*>  cache_priority; // images in the front are the most likely to be removed
    DTagMap metadata;
