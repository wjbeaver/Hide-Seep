...
bool bim::Image5D::fromFile( const std::string &fileName ) {
  if (!fm.sessionIsReading(fileName)) {
    init();
    if (fm.sessionStartRead(fileName.c_str())!=0) return false;
    ...
    for (unsigned int i=0; i<stacks.size(); ++i) stacks[i] = DImageStack();
...
TDimImage *bim::Image5D::imageAt( unsigned int t, unsigned int z ) {
  if (t>=image_size[bim::Image5D::t]) return 0;
  DImageStack *s = this->stackAt(t);
  if (!s) return 0;
  return s->imageAt(z);
}

DImageStack *bim::Image5D::stackAt( unsigned int t ) {
  if (t>=image_size[bim::Image5D::t]) return 0;
  if (t>=stacks.size()) return 0;
  DImageStack *s = &stacks[t];
...
void bim::Image5D::updateCache() {
  ...
  while (memorySize()>maximum_cache_size && cache_priority.size()>1) {
    DImageStack *s = cache_priority.front();
    s->clear();
    cache_priority.pop_front();
  }
}

DImageStack *bim::Image5D::stack() {
  return this->stackAt( this->current_position[bim::Image5D::t] );
}

const DImageStack *bim::Image5D::current() const {
  return &stacks[this->current_position[bim::Image5D::t]];
}

TDimImage *bim::Image5D::image() {
