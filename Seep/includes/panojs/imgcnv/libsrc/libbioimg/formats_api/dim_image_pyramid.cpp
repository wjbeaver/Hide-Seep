...
void DImagePyramid::createFrom( const TDimImage &img ) {
  ...
  unsigned int side = dim::max<unsigned int>( images[0].width(), images[0].height() );
  int max_levels = (int) ceil( dim::log2<double>( side ) );
  int i = 0;
  while ( dim::max<unsigned int>( images[i].width(), images[i].height() ) > (unsigned int) min_width ) {
    xstring x;
