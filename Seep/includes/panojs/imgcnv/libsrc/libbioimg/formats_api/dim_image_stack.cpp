...
bool DImageStack::fromFileManager( TMetaFormatManager *m, const std::vector<unsigned int> &pages ) {
  int res = 0;
  handling_image = true;
  images.clear();
  metadata.clear();
...
double DImageStack::bytesInStack() const {
  double bytes=0;
...
DTagMap resizeMetadata3d( const DTagMap &md, unsigned int w_to, unsigned int h_to, unsigned int d_to, unsigned int w_in, unsigned int h_in, unsigned int d_in ) {
  DTagMap metadata = md;
...
void DImageStack::resize( int w, int h, int d, TDimImage::ResizeMethod method, bool keep_aspect_ratio ) {
  ...
  stack.metadata = resizeMetadata3d( this->metadata, w, h, d, width(), height(), length() );
  *this = stack;
}
