/*******************************************************************************
  ...
  Interpolation filters and application functions
...
inline T BesselOrderOne(const T &xx) {
  ...
  q=sqrt((double) (2.0/(dim::Pi*x)))*(P1<T>(x)*(1.0/sqrt(2.0)*(sin((double) x)-
...
public:
  ...
  void setSupport( const T &s ) {
    _support = s;
    contribution.resize( (unsigned int) ceil(2.0*dim::max<T>(_support,0.5)+5) );
  }
...
public:
  DBesselFilter() { setSupport((T)0.0); }

  inline T function(const T &x) const {
    if (x == 0.0)
      return((T) (dim::Pi/4.0));
    return (T)(BesselOrderOne(dim::Pi*x)/(2.0*x));  
  }
};
...
public:
  DSincFilter() { setSupport((T)0.0); }

  inline T function(const T &x) const {
    if (x == 0.0) return 1.0;
    return (T)(sin(dim::Pi*(double) x)/(dim::Pi*(double) x));
  }
};
...
public:
  DBlackmanFilter() { setSupport((T)1.0); }

  inline T function(const T &x) const {
    return (T)(0.42+0.5*cos(dim::Pi*(double) x)+0.08*cos(2.0*dim::Pi*(double) x));
  }
};
...
public:
  DBlackmanBesselFilter() { setSupport((T)3.2383); }
...
public:
  DBlackmanSincFilter() { setSupport((T)4.0); }
...
public:
  DPointFilter() { setSupport((T)0.0); }
};

template<typename T> 
class DCatromFilter: public DInterpolationFilter<T> {
public:
  DCatromFilter() { setSupport((T)2.0); }
...
public:
  DGaussianFilter() { setSupport((T)1.25); }

  inline T function(const T &x) const {
    return (T)( exp((double) (-2.0*x*x))*sqrt(2.0/dim::Pi) );
  }
};
...
public:
  DHanningFilter() { setSupport((T)1.0); }

  inline T function(const T &x) const {
    return (T)(0.5+0.5*cos(dim::Pi*(double) x));
  }
};
...
public:
  DHammingFilter() { setSupport((T)1.0); }

  inline T function(const T &x) const {
    return (T)(0.54+0.46*cos(dim::Pi*(double) x));
  }
};
...
public:
  DHermiteFilter() { setSupport((T)1.0); }
...
public:
  DLanczosFilter() { setSupport((T)3.0); }
...
#define Q3  ((     - 1.0*B- 6.0*C)/6.0)
  ...
  DMitchellFilter() { setSupport((T)2.0); }
...
public:
  DQuadraticFilter() { setSupport((T)1.5); }
...
public:
  DTriangleFilter() { setSupport((T)1.0); }
                              ...
                              DInterpolationFilter<Tw> &filter, const Tw blur )
{
  ...
  scale = (Tw)( blur * dim::max<double>( 1.0/x_factor, 1.0 ) );
  ...
  scale = (Tw)(1.0/scale);
  if (support*2+2 > filter.contribution.size()) filter.contribution.resize( (unsigned int)(support*2+2) );

  for (x=0; x<w_to; ++x) {
    center  = (Tw)  (x+0.5)/x_factor;
    start   = (int) (dim::max<double>(center-support,0.0) + 0.5);
    stop    = (int) (dim::min<double>(center+support,w_in+0.5));
    density = 0.0;
    ...
    p = psrc + filter.contribution[0].pixel;
    q = pdest + x;
    for (y=0; y<h_to; ++y) {
      pixel = 0;
      unsigned int yl = y*offset_in;
      for (i=0; i<n; ++i) {
        j = yl + (filter.contribution[i].pixel - filter.contribution[0].pixel);
      ...
      if (std::numeric_limits<Td>::is_integer && !std::numeric_limits<Tw>::is_integer)
        *q = dim::trim<Td, Tw>( dim::round<Tw>(pixel), TdMin, TdMax );
      else
        *q = dim::trim<Td, Tw>( pixel, TdMin, TdMax );
                            ...
                            DInterpolationFilter<Tw> &filter, const Tw blur )
{
  ...
  scale = (Tw)(blur * dim::max<double>( 1.0/y_factor, 1.0 ) );
  support = scale * filter.support();
  ...
  for (y=0; y<h_to; ++y) {
    center  = (Tw)  (y+0.5)/y_factor;
    start   = (int) (dim::max<double>(center-support,0.0) + 0.5);
    stop    = (int) (dim::min<double>(center+support, h_in+0.5));
    density = 0.0;
    ...
    p = psrc + (offset_to * (filter.contribution[0].pixel));
    q = pdest + (offset_to * y);
    for (x=0; x<w_to; ++x) {
      pixel = 0;
      for (i=0; i<n; ++i) {
        j = ((filter.contribution[i].pixel-filter.contribution[0].pixel)*offset_in + x);
        alpha = filter.contribution[i].weight;
        pixel += alpha*p[j];
      } // for i to n
                         ...
                         DInterpolationFilter<Tw> &filter, const Tw blur )
{
  Tw x_factor = (Tw) w_to /(Tw) w_in;
  Tw y_factor = (Tw) h_to /(Tw) h_in;
  Tw x_support = (Tw)( blur * dim::max<double>( 1.0/x_factor, 1.0*filter.support() ) );
  Tw y_support = (Tw)( blur * dim::max<double>( 1.0/y_factor, 1.0*filter.support() ) );
  if (dim::max<Tw>(x_support,y_support) > filter.support() )
    filter.setSupport( dim::max<Tw>(x_support,y_support) );
...
void image_resample_NN ( T *pdest, unsigned int w_to, unsigned int h_to, unsigned int offset_to,
  ...
  for (y=0; y<h_to; y++) {
    for (x=0; x<w_to; x++) {
      xn = dim::trim<unsigned int, Tw>(ceil(x*ratio_h), 0, w_in-1 );
      yn = dim::trim<unsigned int, Tw>(ceil(y*ratio_v), 0, h_in-1 );
      p = psrc + (offset_in * yn);
      q[x] = p[xn];      
    } // x
    q += offset_to;
                              ...
                              DInterpolationFilter<Tw> &filter, const Tw blur )
{
  ...
  for (x=0; x<new_size; ++x) {
    center  = (Tw)  (x+0.5)/x_factor;
    start   = (int) (dim::max<double>(center-support,0.0) + 0.5);
    stop    = (int) (dim::min<double>(center+support,src.size()+0.5));
    density = 0.0;
    ...
    q[x] = dim::trim<Td, Tw>( pixel, TdMin, TdMax );
...
std::vector<T> vector_resample_NN ( const std::vector<T> &src, unsigned int new_size ) {
  ...
  for (x=0; x<new_size; x++) {
    xn = dim::trim<unsigned int, Tw>(ceil(x*ratio), 0, src.size()-1 );
    q[x] = src[xn];      
  } // x
  return q;
}
