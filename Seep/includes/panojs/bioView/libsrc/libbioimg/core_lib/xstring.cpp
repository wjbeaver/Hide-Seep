...
xstring &xstring::removeSpacesLeft() {
  int p = this->find_first_not_of(" ");
...
xstring &xstring::removeSpacesRight() {
  int p = this->find_last_not_of(" ");
  if (p != std::string::npos) {
    this->resize(p+1, ' ');
  }
  return *this;
...
xstring &xstring::removeSpacesBoth() {
  removeSpacesLeft();
  removeSpacesRight();
  return *this;
}
