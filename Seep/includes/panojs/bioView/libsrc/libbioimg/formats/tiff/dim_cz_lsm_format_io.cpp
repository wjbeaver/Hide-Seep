...
DIM_UINT append_metadata_lsm (TDimFormatHandle *fmtHndl, DTagMap *hash ) {
  ...
  hash->append_tag( "image_num_z", lsm->z_slices );
  hash->append_tag( "image_num_t", lsm->t_frames );
  hash->append_tag( "image_num_c", lsm->ch );

  hash->append_tag( "pixel_resolution_x", lsm->res[0]*1000000 );
  hash->append_tag( "pixel_resolution_y", lsm->res[1]*1000000 );
  hash->append_tag( "pixel_resolution_z", lsm->res[2]*1000000 );
  hash->append_tag( "pixel_resolution_t", lsm->res[3] );
  ...
  for (int i=0; i<lsm->scan_info_entries.size(); i++) {
    ...
    key = xstring("custom/") + lsm->scan_info_entries[i].path + lsm->key_names[lsm->scan_info_entries[i].entry_type];
    ...
    if (lsm->scan_info_entries[i].entry_type == 0x10000004) hash->append_tag( "objective", val );
    if (lsm->scan_info_entries[i].entry_type == 0x10000001) hash->append_tag( "label", val );
  }
