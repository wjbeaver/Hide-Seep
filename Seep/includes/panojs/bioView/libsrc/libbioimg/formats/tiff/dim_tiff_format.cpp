...
void getCurrentPageInfo(DTiffParams *tiffParams) {
  ...
  if (tiffParams->subType!=tstFluoview && tiffParams->subType!=tstPsia)
...
void getImageInfo(DTiffParams *tiffParams) {
  ...
  if (fluoviewIsTiffValid( tiffParams )) {
...
TDimFormatItem dimTiffItems[D_TIFF_NUM_FORMATS] = {
  ...
  },
  {
    ...
    "Carl Zeiss LSM5 File Format", // Long format name
