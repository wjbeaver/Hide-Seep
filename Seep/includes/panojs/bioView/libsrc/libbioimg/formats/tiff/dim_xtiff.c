...
{
  DIM_XTiff *lXtiff = (DIM_XTiff *) tif->tif_clientinfo->data;
...
void _XTIFFPostInitialize(TIFF *tif)
{
  ...
  if (tif->tif_clientinfo->data == NULL)
    tif->tif_clientinfo->data = _TIFFmalloc( sizeof(DIM_XTiff) );
  dim_xtiff = tif->tif_clientinfo->data;

  dim_xtiff->parent_vsetfield = NULL;
  dim_xtiff->parent_vgetfield = NULL;
  dim_xtiff->xd_num_uic2      = 0;
  dim_xtiff->xd_uic2          = NULL;
  ...
  dim_xtiff->parent_vsetfield = tif->tif_tagmethods.vsetfield;
  tif->tif_tagmethods.vsetfield = _XTIFFVSetField;

  dim_xtiff->parent_vgetfield = tif->tif_tagmethods.vgetfield;
