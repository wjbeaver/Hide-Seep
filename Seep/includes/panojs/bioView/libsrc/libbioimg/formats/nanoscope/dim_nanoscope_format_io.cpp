...
static int read_nanoscope_image(TDimFormatHandle *fmtHndl) {
  ...
  DNanoscopeImg nimg = nanoPar->imgs.at(page);
  ...
  for (int sample=0; sample<nanoPar->channels; ++sample) {
    ...
    if (dimRead( fmtHndl, img->bits[sample], img_size_bytes, 1 ) != 1) return 1;
    if ( (img->i.depth == 16) && (dimBigendian) ) 
      dimSwapArrayOfShort((DIM_UINT16*) img->bits[sample], img_size_bytes/2);
    
    // now convert from SHORT to USHORT
    if (img->i.depth == 16) {
      DIM_INT16 *pS  = (DIM_INT16 *)  img->bits[sample];
      DIM_UINT16 *pU = (DIM_UINT16 *) img->bits[sample];
    ...
    for (int y=0; y<floor(info->height/2.0); y++) {
      dimProgress( fmtHndl, y, (long) floor(info->height/2.0), "Reading NANOSCOPE" );
...
DIM_UINT read_nanoscope_metadata (TDimFormatHandle *, int , int , int )
{
  return 0;
}
...
DIM_UINT nanoscope_append_metadata (TDimFormatHandle *fmtHndl, DTagMap *hash ) {
  if (fmtHndl == NULL) return 1;
  if (fmtHndl->internalParams == NULL) return 1;
  if (!hash) return 1;
  DNanoscopeParams *par = (DNanoscopeParams *) fmtHndl->internalParams; 
  ...
  if (line) {
    ...
    hash->append_tag( "date_time", xs );
  }

  // channel names
  if (par->channels>1)
    for (int i=0; i<par->imgs.size(); ++i)
      hash->set_value( xstring::xprintf("channel_%d_name", i), par->imgs.at(i).data_type );
  else
    for (int i=0; i<par->imgs.size(); ++i)
      hash->set_value( xstring::xprintf("image_%d_name", i), par->imgs.at(i).data_type );

  // Parse some of the notes
  hash->parse_ini( par->metaText, ":", "custom/" );
