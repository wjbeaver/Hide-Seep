<?php
// summary
// Test file to handle GPX uploads
//
...
function getTrackType($filename) {
...
//
$upload_path = "data/GPX/"; // where track will be uploaded, relative to this file
$download_path = "Seep/data/GPX/"; // same folder as above, but relative to the HTML file
...
$hash = $postdata["UPLOADIDTrackUpload"];
...
if ( isset($_FILES[$fieldName]) || isset($_FILES['uploadedfileFlash'])) {
    ...
    $type = getTrackType($file);
    trace("file: " . $file ." ".$type);
    ...
    while(isset($_FILES['uploadedfile'.$cnt])){
        trace("IFrame multiple POST");
        $trackHash[$cnt] = UUID::v4();
        $moveName = $trackHash[$cnt].".gpx";
  ...
  trace(count($_POST['uploadedfiles'])." ");
    ...
    for($i=0;$i<$len;$i++){
        ...
        if ($moved) {
            ...
            $gpx_file = file_get_contents($file);

            $htmldata[$cnt]["track"] = $gpx_file;
    ...
    trace($_FILES);
    $htmldata = array("Message" => "Submit Successful - No Tracks Sent");
}
